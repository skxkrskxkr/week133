...
int main(void)
{
	...
	GLuint VertexArrayID; // memory space make
	...
	glm::vec3 tri_center(0.0);

	for (int i = 0; i < triangle_vertices.size(); i++)
	{
		tri_center += triangle_vertices[i];
	}
	tri_center.x /= (float)triangle_vertices.size();
	tri_center.y /= (float)triangle_vertices.size();
	tri_center.z /= (float)triangle_vertices.size();
	...
	do {
		...
		//Model = glm::rotate(Model, glm::radians(0.1f), glm::vec3(0.0, 0.0, 1.0));
		//Model = glm::translate(tri_center)*glm::rotate(glm::mat4(), glm::radians(1.0f), glm::vec3(0, 0, 1)) * glm::translate(-tri_center) * Model;
		...
		// Draw the triangle !
		glDrawArrays(GL_LINE_LOOP, 0, 3); // 3 vertices -> 3lines
		glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer2);
		glVertexAttribPointer(
			0,                  // attribute 0. No particular reason for 0, but must match the layout in the shader.
			3,                  // size
		...
		glfwPollEvents(); // Å°ÀÔ·Â
		if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS)
		{
			Model = glm::translate(dir * 0.005f) * Model;
			tri_center += dir * 0.005f;
